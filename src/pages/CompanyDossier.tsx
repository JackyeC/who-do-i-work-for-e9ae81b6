  ...
  const evidenceRecords: EvidenceRecord[] = useMemo(() => {
    ...
    if ((company.total_pac_spending ?? 0) > 0) {
      records.push({
        ...
        sourceUrl: "https://www.opensecrets.org/political-action-committees-pacs",
        sourceName: "OpenSecrets / FEC",
      });
    }
    ...
    if ((company.lobbying_spend ?? 0) > 0) {
      records.push({
        eventType: "Lobbying Expenditure",
        category: "Lobbying",
        date: null,
    ...
    (eeocCases || []).forEach((c: any) => {
      records.push({
        eventType: "EEOC Filing",
        category: "Enforcement & EEOC",
        date: c.filing_date || c.created_at || null,
        amount: c.settlement_amount ?? null,
        description: c.description || c.case_summary || `EEOC case filed against ${company.name}.`,
        sourceUrl: c.source_url || "https://www.eeoc.gov/",
        sourceName: "EEOC",
      });
    });
