...
import { ValuesOnboardingStep } from "@/components/onboarding/ValuesOnboardingStep";
...
export function FirstLoginOnboarding({ onComplete }: FirstLoginOnboardingProps) {
  ...
  const handleValuesComplete = async (selectedValues: string[]) => {
    // Save quick-selected values as high-importance (80)
    if (user && selectedValues.length > 0) {
      try {
        const payload: Record<string, any> = { user_id: user.id };
        selectedValues.forEach(key => {
          payload[`${key}_importance`] = 80;
        });
        await (supabase as any)
          .from("user_values_profile")
          .upsert(payload, { onConflict: "user_id" });
      } catch (err) {
        console.error("Failed to save quick values:", err);
      }
    }
    setStep(step + 1);
  };
      ...
      >
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="p-8"
          >
            {/* Values step - custom component */}
            {current.id === "values" ? (
              <ValuesOnboardingStep
        ...
        {/* Footer - hide for values step (it has its own buttons) */}
        {current.id !== "values" && (
