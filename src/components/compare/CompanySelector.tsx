...
export function CompanySelector({ label, company, search, suggestions, onSearchChange, onSelect, onClear }: CompanySelectorProps) {
    ...
    return (
      <div className="bg-card border border-border p-5 relative group hover:border-primary/30 transition-all">
        <div className="absolute top-3 right-3">
          <button onClick={onClear} className="font-mono text-xs tracking-wider uppercase text-muted-foreground hover:text-primary transition-colors">
            Change
          </button>
        </div>
        <div className="font-mono text-xs tracking-[0.2em] uppercase text-primary mb-2">{label}</div>
  ...
  return (
    <div>
      <div className="font-mono text-xs tracking-[0.2em] uppercase text-muted-foreground mb-2">{label}</div>
            ...
            {showNoResults && (
              <div className="px-4 py-3 text-center">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 font-mono text-xs tracking-wider uppercase text-primary hover:underline"
                >
