...
import {
  ...
  Shield, GitMerge, ExternalLink, Search, Info,
  Megaphone, Landmark, BarChart3, Eye,
} from "lucide-react";
...
const IMPACT_COPY = {
  ...
  concentrated: {
    world: "When decision-making power and external influence are concentrated in one leader, company direction may reflect individual priorities more than collective leadership alignment.",
    you: "If you value shared leadership accountability, this concentration is something to look at closely before joining.",
  },
  distributed: {
    world: "Distributed political engagement across leadership suggests systemic alignment rather than individual initiative. Policy positions may reflect shared leadership consensus.",
    you: "This pattern suggests the political stance is built into the culture, not just one person's preference. Consider whether that alignment matches yours.",
  },
  passive: {
    world: "Minimal political activity may indicate genuine neutrality, or spending below disclosure thresholds. Other signals — lobbying, board connections — may reveal more.",
    you: "Neutral political posture can be a positive signal if you prefer employers that stay out of politics. But verify through other lenses.",
  },
};
        ...
        <div className="p-5 space-y-6">
          ...
          <Section icon={DollarSign} label="SPENDING OVERVIEW">
            ...
            <AnimatePresence>
              {expandedSpending === "pac" && (
                <DrillPanel>
                  <DrillHeader title="PAC Spending Breakdown" sourceLabel="FEC.gov" sourceUrl="https://www.fec.gov/data/" />
                  <p className="text-xs text-muted-foreground leading-relaxed mb-3">
                    Political Action Committee contributions are pooled funds from corporate employees and executives, directed to candidates, parties, and ballot measures.
                  </p>
                  <div className="grid grid-cols-2 gap-2 mb-3">
                    <MiniStat label="Total PAC" value={fmt(totalPacSpending)} />
                    <MiniStat label="Recipients on record" value={String(deduped.length)} />
                  </div>
                  {deduped.length > 0 && (
                    <div className="space-y-1 max-h-40 overflow-y-auto">
                      <p className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground mb-1">Top Recipients</p>
                  ...
                  <Link to={`/follow-the-money`} className="inline-flex items-center gap-1 text-xs text-primary hover:underline mt-2">
                    Explore full money trail <ChevronRight className="w-3 h-3" />
                  </Link>
                </DrillPanel>
              )}
            </AnimatePresence>
            ...
            <AnimatePresence>
              {expandedSpending === "lobby" && (
                <DrillPanel>
                  <DrillHeader title="Lobbying Activity" sourceLabel="OpenSecrets / LDA" sourceUrl="https://www.opensecrets.org/federal-lobbying" />
                  <p className="text-xs text-muted-foreground leading-relaxed mb-3">
                    Lobbying disclosures filed under the Lobbying Disclosure Act show payments to firms and individuals hired to influence federal legislation, rulemaking, and executive branch decisions.
                  </p>
                  <div className="grid grid-cols-2 gap-2 mb-3">
                    <MiniStat label="Total Lobbying" value={fmt(lobbyingSpend)} />
                    <MiniStat label="Status" value={lobbyingSpend > 1_000_000 ? "Significant" : "Moderate"} />
                  </div>
                  <ImpactBlock
              ...
              <div className="mt-3 space-y-2">
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs mt-1">
                    ...
                    {otherAmount > 0 && <span style={{ color: "#888780" }}>Other: {fmt(otherAmount)}</span>}
                  </div>
                </button>
                ...
                <AnimatePresence>
                  {expandedParty && (
                    <DrillPanel>
                      <DrillHeader title="Party Distribution Detail" sourceLabel="FEC Individual Contributions" sourceUrl="https://www.fec.gov/data/receipts/individual-contributions/" />
            ...
            {totalSpend > 0 && (
              <ImpactBlock
                world={IMPACT_COPY.spending.world}
                you={IMPACT_COPY.spending.you}
                className="mt-3"
              />
            )}
          </Section>
          ...
          <Section icon={Crown} label="TOP INFLUENCERS">
                ...
                <div key={entity.primaryId}>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <span className="text-[10px] text-primary opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">View profile</span>
                      <ChevronRight className={cn("w-4 h-4 text-muted-foreground transition-transform", selectedLeader === entity.primaryId && "rotate-90")} />
                    </div>
                  </button>

                  {/* Full profile drill-down */}
                  <AnimatePresence>
                      ...
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                        <div className="ml-12 rounded-lg border border-primary/20 bg-primary/[0.02] p-4 mb-2 space-y-4">
                          ...
                          {entity.contributionCount > 1 && (
                            <p className="text-xs text-muted-foreground">{entity.contributionCount} contribution record{entity.contributionCount !== 1 ? "s" : ""} aggregated</p>
                          )}

                          {/* Contribution history — show matched candidates */}
                          {entity.totalDonations > 0 && deduped.length > 0 && (
                            <div>
                              <p className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground mb-1.5">Contribution History</p>
                          ...
                          {/* Related signals — pattern tags */}
                          <div>
                            <p className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground mb-1.5">Related Signals</p>
                            <div className="flex flex-wrap gap-1.5">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Button variant="ghost" size="sm" className="h-7 text-xs gap-1" asChild>
                              <Link to="/follow-the-money"><BarChart3 className="w-3 h-3" /> Follow the money</Link>
                            </Button>
                          </div>
                        </div>
                      </motion.div>
            ...
            {activeContributors.length > 0 && (
              <ImpactBlock
                world={IMPACT_COPY[impactKey].world}
                you={IMPACT_COPY[impactKey].you}
                className="mt-3"
              />
            )}
          </Section>
            ...
            <Section icon={Users} label={`ALL LEADERSHIP (${taggedEntities.length})`}>
              <div className="space-y-1">
                {displayLeaders.map((entity) => (
                  <div key={entity.primaryId} className="flex items-center justify-between py-1.5 px-1 rounded hover:bg-muted/30 transition-colors cursor-pointer"
                    onClick={() => setSelectedLeader(selectedLeader === entity.primaryId ? null : entity.primaryId)}>
                    <div className="flex items-center gap-2 min-w-0">
                      <span className="text-[10px] font-mono text-muted-foreground w-4 shrink-0">{entity.source === "executive" ? "E" : "B"}</span>
                      <span className="text-xs text-foreground truncate hover:text-primary transition-colors">{entity.displayName}</span>
                      {entity.mergedRecords.length > 1 && (
              ...
              {flags.map((flag, i) => (
                <div key={i}>
                  <button
                    ...
                    className={cn(
                      "flex items-center gap-2 w-full text-left rounded-lg px-3 py-2 transition-all cursor-pointer border",
                      expandedPattern === i
                  ...
                  >
                    ...
                    <span className="text-[10px] text-primary opacity-0 group-hover:opacity-100 ml-auto">Explore</span>
        ...
        {deduped.length > 0 && (
          <div className="border-t border-border">
            <button
              ...
              className="w-full flex items-center gap-2 px-5 py-3 text-xs text-muted-foreground hover:text-foreground transition-colors cursor-pointer"
            >
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        {hasDetail && <span className="text-[9px] text-primary">{isActive ? "Close" : "Explore"}</span>}
      </div>
...
function DrillHeader({ title, sourceLabel, sourceUrl }: { title: string; sourceLabel: string; sourceUrl: string }) {
  return (
...
function ImpactBlock({ world, you, className }: { world: string; you: string; className?: string }) {
  return (
    <div className={cn("space-y-2 border-l-2 border-primary/20 pl-3", className)}>
      <div>
        <p className="text-[10px] font-mono uppercase tracking-wider text-primary/70 mb-0.5">Impact on work</p>
