/**
 * WhoDoI Trail — Center board with premium game-like feel.
 * Less dashboard, more evidence wall.
 */
...
const CATEGORY_LABELS: Record<CardCategory, { label: string; icon: string; accent: string; flavor: string }> = {
  receipt: { label: "Money Trail", icon: "💰", accent: "#F2C14E", flavor: "Where the dollars go tells the real story" },
  person: { label: "Key People", icon: "👤", accent: "#9B7BFF", flavor: "The names behind the decisions" },
  claim: { label: "Public Claims", icon: "📢", accent: "#39C0BA", flavor: "What they say vs. what they do" },
  signal: { label: "Worker Signals", icon: "📡", accent: "#FF6B6B", flavor: "Intel from the inside" },
  network: { label: "Hidden Networks", icon: "🕸️", accent: "#FF9F43", flavor: "The invisible web of influence" },
  reveal: { label: "Reveals", icon: "🔓", accent: "#63D471", flavor: "The pieces fall into place" },
};
...
export function TrailBoard() {
  ...
  // Pre-board phases — immersive intro
      ...
      <div className="flex-1 flex items-center justify-center p-8 relative overflow-hidden"
        ...
        <div className="text-center max-w-lg space-y-6 relative">
          <div className="w-24 h-24 rounded-3xl mx-auto flex items-center justify-center text-5xl relative"
            ...
            {state.phase === "intro" ? "🔍" : state.phase === "act1" ? "🧠" : "⚖️"}
          ...
          <div>
            <p className="text-[10px] font-mono uppercase tracking-[0.25em] mb-3" style={{ color: "#F2C14E" }}>
              {state.phase === "intro" ? "Act 1 of 3" : state.phase === "act1" ? "Act 2 of 3" : "Act 3 of 3"}
            </p>
            <h2 className="text-2xl font-bold mb-2" style={{ color: "#F5F1E8" }}>
              {state.phase === "intro" ? "Begin Your Investigation" :
               state.phase === "act1" ? "What's Your Theory?" :
               "What Matters Most to You?"}
            </h2>
            <p className="text-sm leading-relaxed max-w-sm mx-auto" style={{ color: "#B9C0CC" }}>
              {state.phase === "intro"
                ? "Every company has a public record. Choose a trail to start pulling the thread."
                : state.phase === "act1"
                ? "You've found the first clues. Now form a theory about the pattern you're seeing."
                : "Almost there. As a worker, what do you care about most? This shapes your final verdict."
              }
            </p>
          </div>
          ...
          {state.phase !== "intro" && revealedCards.size > 0 && (
            <div className="pt-2">
              <p className="text-[10px] font-mono uppercase tracking-[0.2em] mb-3 flex items-center justify-center gap-2"
                ...
                {revealedCards.size} clue{revealedCards.size !== 1 ? "s" : ""} in your file
              </p>
  ...
  return (
    <div className="flex-1 overflow-y-auto" style={{ background: "#0F1118" }}>
      {/* Connector legend — minimal, game-like */}
      ...
      <div className="px-5 pb-8 pt-4 space-y-8">
        {/* Connections panel — game mission style */}
          ...
          <div className="rounded-2xl overflow-hidden"
            ...
            <div className="px-4 py-3 flex items-center justify-between"
              ...
              <h3 className="text-xs font-bold uppercase tracking-wider flex items-center gap-2"
                style={{ color: "#F2C14E" }}>
                <Zap className="w-3.5 h-3.5" /> Connect the Evidence
              </h3>
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[10px] font-mono" style={{ color: "#B9C0CC" }}>
                  / {availableConnections.length} found
                </span>
              </div>
            </div>
        ...
        {groupedCards.map(group => {
          ...
          return (
            <div key={group.category}>
              {/* Category header — editorial, not dashboard */}
