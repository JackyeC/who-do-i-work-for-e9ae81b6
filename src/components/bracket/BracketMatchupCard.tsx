    ...
    >
      ...
      <div className="relative z-10">
        <div className="flex items-center justify-between mb-1">
          <span className="font-mono text-xs text-muted-foreground">#{seed}</span>
          {isVoted && <Check className="w-3 h-3 text-primary" />}
        </div>
        <div className="text-lg mb-0.5">{brand.emoji}</div>
        <div className="font-bold text-xs text-foreground leading-tight mb-0.5">{brand.name}</div>
        <div className="font-mono text-xs tracking-wider uppercase text-muted-foreground">{brand.tagline}</div>
        {userVote && (
          <div className="mt-1.5 font-mono text-xs font-bold text-primary">{pct}%</div>
        )}
      </div>
    </button>
  );
    ...
    <div className="border border-border bg-card hover:border-primary/20 transition-all">
      ...
      {matchup.tension && (
        <div className="px-3 py-1.5 border-b border-border bg-muted/10 flex items-center justify-center">
          <span className="font-mono text-xs tracking-[0.15em] uppercase text-accent-foreground/70">
        ...
        <div className="border-t border-border bg-muted/10 px-3 py-3 space-y-2 animate-in fade-in-0 slide-in-from-bottom-2 duration-300">
          ...
          {matchup.operatingContext && (
            <div className="bg-card border border-border/50 rounded px-3 py-2 space-y-1">
              <p className="font-mono text-xs tracking-[0.2em] uppercase text-muted-foreground">How they tend to operate</p>
