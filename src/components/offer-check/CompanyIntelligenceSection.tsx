  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [
        ...
        supabase.from("company_worker_sentiment").select("sentiment, ai_summary").eq("company_id", companyId).limit(10),
      ...
      return {
        executives: (execs.data ?? []) as any[] as { name: string; title: string; total_donations: number }[],
        warns: (warns.data ?? []) as any[] as { notice_date: string; employees_affected: number; event_type: string }[],
        sentiments: (sentiment.data ?? []) as any[] as { sentiment: string; ai_summary: string | null }[],
